...
describe('X402 Nansen Smart Money Netflows TRUE End-to-End', () => {
  ...
  beforeAll(async () => {
    console.log('\n' + '='.repeat(70));
    console.log('🚀 TRUE END-TO-END TEST: X402 Nansen Smart Money Netflows');
  ...
  test('Complete flow: User asks → AI needs payment → Pay Nansen → AI responds', async () => {
    ...
    // ============================================
    // STEP 2: Send user message that triggers Nansen Smart Money Netflows
    // ============================================
    console.log('📋 Step 2: Sending message that requires Nansen smart money data...');
    console.log('   User asks: "What are smart money net flows on Ethereum in the last 24 hours?"');
    
    const response1 = await sendChatMessage(
      "What are smart money net flows on Ethereum in the last 24 hours?",
      conversationId,
      authToken
    );
    ...
    if (!parsed1.hasPaymentRequirement) {
      console.warn('\n⚠️  AI did not trigger Nansen Smart Money Netflows tool');
    ...
    expect(paymentReq.toolName).toBe('nansenSmartMoneyNetflows');
    expect(paymentReq.apiUrl).toContain('nansen');
    expect(paymentReq.apiUrl).toContain('smart-money/netflow');
    ...
    console.log('   - Get real smart money data back');
    ...
    const messagesWithResult = [
      {
        role: 'user' as const,
        content: "What are smart money net flows on Ethereum in the last 24 hours?",
        parts: [{
          type: 'text',
          text: "What are smart money net flows on Ethereum in the last 24 hours?"
        }]
      },
      {
    ...
    // AI should mention smart money or flows
    const mentionsSmartMoney = responseLower.includes('smart money') || responseLower.includes('flow');
    console.log('   Mentions smart money/flows:', mentionsSmartMoney);
    
    // AI should talk about tokens or trading
    const mentionsTokens = responseLower.match(/token|trading|buying|selling/);
    ...
    console.log('  ✅ AI determines it needs Nansen smart money data');
    console.log('  ✅ AI returns payment requirement');
    console.log('  ✅ Test executes REAL payment to REAL Nansen API');
    console.log('  ✅ Gets REAL smart money flow data back');
