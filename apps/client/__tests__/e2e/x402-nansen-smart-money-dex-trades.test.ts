...
describe('X402 Nansen Smart Money DEX Trades TRUE End-to-End', () => {
  ...
  beforeAll(async () => {
    console.log('\n' + '='.repeat(70));
    console.log('🚀 TRUE END-TO-END TEST: X402 Nansen Smart Money DEX Trades');
  ...
  test('Complete flow: User asks → AI needs payment → Pay Nansen → AI responds', async () => {
    ...
    // ============================================
    // STEP 2: Send user message that triggers Nansen Smart Money DEX Trades
    // ============================================
    console.log('📋 Step 2: Sending message that requires Nansen smart money DEX trades data...');
    console.log('   User asks: "What are the recent DEX trades by smart money on Ethereum in the last 24 hours?"');
    
    const response1 = await sendChatMessage(
      "What are the recent DEX trades by smart money on Ethereum in the last 24 hours?",
      conversationId,
      authToken
    );
    ...
    if (!parsed1.hasPaymentRequirement) {
      console.warn('\n⚠️  AI did not trigger Nansen Smart Money DEX Trades tool');
    ...
    expect(paymentReq.toolName).toBe('nansenSmartMoneyDexTrades');
    expect(paymentReq.apiUrl).toContain('nansen');
    expect(paymentReq.apiUrl).toContain('smart-money/dex-trades');
    ...
    console.log('   - Get real smart money DEX trades data back');
    ...
    const messagesWithResult = [
      {
        role: 'user' as const,
        content: "What are the recent DEX trades by smart money on Ethereum in the last 24 hours?",
        parts: [{
          type: 'text',
          text: "What are the recent DEX trades by smart money on Ethereum in the last 24 hours?"
        }]
      },
      {
    ...
    // AI should mention smart money or trades/dex
    const mentionsSmartMoney = responseLower.includes('smart money') || responseLower.includes('trade') || responseLower.includes('dex');
    console.log('   Mentions smart money/trades/dex:', mentionsSmartMoney);
    
    // AI should talk about tokens or trading activity
    const mentionsTokens = responseLower.match(/token|trading|swap|buy|sell/);
    console.log('   Mentions tokens/trading:', !!mentionsTokens);
    ...
    console.log('  ✅ AI determines it needs Nansen smart money DEX trades data');
    console.log('  ✅ AI returns payment requirement');
    console.log('  ✅ Test executes REAL payment to REAL Nansen API');
    console.log('  ✅ Gets REAL smart money DEX trades data back');
